...
def save_progress(last_index):
    with open("./single_train_progress", "w") as f:
        json.dump({"last_trained_index": last_index}, f)

def load_progress():
    if os.path.exists("./single_train_progress"):
        with open("./single_train_progress", "r") as f:
...
def remove_progress():
    if os.path.exists("./single_train_progress"):
        os.remove("./single_train_progress")
        os.remove("./single_dqn_model_temp.pth")
    else:
...
def train_dqn(cars: list[dict], rd:RequestDetail, num_chargers:int = None, num_episodes=2000, period_length=48):
    ...
    if os.path.isfile("single_dqn_model.pth"):
        finished = True
    ...
    if finished is False and not len(train_periods) == last_trained_index:
        ...
        for i, (prices_48, times_48) in enumerate(train_periods):
            ...
            agent.save("single_dqn_model_temp.pth")

        if (isinstance(agent, DQNAgent)):
        # Save the trained model
            agent.save("single_dqn_model.pth")
